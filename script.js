async function fetchData() {
    try {
      ...
      for (let i = 0; i < emojiAr.length; i++) {
        ...
        col.setAttribute(
          "class",
          "col-sm-6 col-md-5 col-lg-4 col-xl-4 col-div"
        );
...
const heading1 = document.createElement("h1");
heading1.setAttribute('class', 'text-center text-uppercase py-3');
heading1.setAttribute('id', 'title');
heading1.innerText = "EmojiHub";

const row = document.createElement("div");
row.setAttribute('class', 'row py-2 d-flex justify-content-evenly bg-info');

for(let i=0;i<data.length;i++){
    let col = document.createElement("div");
    col.setAttribute('class', 'col-12 col-md-4 col-lg-4 col-xl-4 mb-4');

    let card = document.createElement("div");
    card.setAttribute('class', 'card my-3 text-center bg-white text-capitalize h-100');
    card.setAttribute('id', data[i].emojiTitle);

    let cardHeader = document.createElement("div");
    cardHeader.setAttribute('class', 'card-header');

    let cardTitle = document.createElement("h5");
    cardTitle.setAttribute('class', 'card-title w-100 text-center py-3 m-0');
    cardTitle.innerText = data[i].emojiTitle;
    ...
    let cardImg = document.createElement("p");
    cardImg.setAttribute('class', 'fa-5x');
    cardImg.innerHTML = data[i].emojiImg;

    let emojiDetailsDiv = document.createElement("div");
    emojiDetailsDiv.setAttribute('class', 'card-text');

    emojiDetailsDiv.innerHTML = `
                <p><strong>Name</strong> : ${data[i].emojiTitle}</p>
                <p><strong>Catagory</strong>  : ${data[i].emojiCategory}</p>
                <p><strong>Group</strong> : ${data[i].emojiGroup}</p>
                <p><strong>HTML-Code</strong> : ${data[i].emojiHtmlCode}</p>
                <p><strong>Uni-Code</strong> : ${data[i].emojiUniCode}</p>
            `;
    ...
    copyButton.setAttribute('onClick', `copyHtmlCode(this, '${data[i].emojiImg}')`);
    copyButton.innerText = `Copy Html Code`;
